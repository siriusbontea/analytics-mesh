...
def create_app(config: PlaneConfig | None = None, proxy: NodeProxy | None = None) -> FastAPI:
    ...
    def run_query(request: PlaneQueryRequest) -> PlaneQueryResponse:
        ...
        if status == "failed":
            raise HTTPException(
                status_code=400,
                detail={"message": result.receipt.error or "query failed", "job": job.model_dump(mode="json"), "receipt": result.receipt.model_dump(mode="json")},
            )
        return PlaneQueryResponse(job=job, artifact=result.artifact, receipt=result.receipt)
